...
def main():
    ...
    log(f"Starting trg v0.15.0 rigorous qualification suite in: {repo_root}")
    ...
    # Validate exact 0.15.0 identity on both binaries
    r_pkg_ver = run_cmd([str(pkg_bin_path), "-V"])
    assert r_pkg_ver.stdout.strip() == "trg 0.15.0 (Toka)", f"Expected 'trg 0.15.0 (Toka)', got '{r_pkg_ver.stdout.strip()}'"

    r_dir_ver = run_cmd([str(direct_bin_path), "-V"])
    assert r_dir_ver.stdout.strip() == "trg 0.15.0 (Toka)", f"Expected 'trg 0.15.0 (Toka)', got '{r_dir_ver.stdout.strip()}'"

    # Use package build artifact as the primary qualification subject
    trg = str(pkg_bin_path)
    fixtures_dir = repo_root / "tests" / "fixtures"

    # Test 1: Help & Version exact 0.15.0
    log("Test 1: Help & Version flags (exact 0.15.0 release identity)")
    r = run_cmd([trg, "--help"])
    assert r.returncode == 0
    assert "trg 0.15.0 - Fast, agent-friendly code search tool" in r.stdout, f"Unexpected help: {r.stdout}"

    r = run_cmd([trg, "--version"])
    assert r.returncode == 0
    assert r.stdout.strip() == "trg 0.15.0 (Toka)", f"Unexpected version: {r.stdout}"
    assert r.returncode == 0
    ...
    assert "trg 0.15.0" in r_re_ctx.stdout
    ...
    try:
        ...
        assert resp1["result"]["serverInfo"]["version"] == "0.15.0"
...
"""
    ...
    with tempfile.TemporaryDirectory(prefix="trg_test_135_") as td:
        ...
        assert len(lines) == 3
        # 1. initialize
        assert lines[0]["result"]["serverInfo"]["version"] == "0.15.0"
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # 1. initialize
        assert resps[0]["result"]["serverInfo"]["version"] == "0.15.0"
