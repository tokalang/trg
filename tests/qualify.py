...
def main():
    ...
    log(f"Starting trg v0.16.0 rigorous qualification suite in: {repo_root}")
    ...
    # Validate exact 0.16.0 identity on both binaries
    r_pkg_ver = run_cmd([str(pkg_bin_path), "-V"])
    assert r_pkg_ver.stdout.strip() == "trg 0.16.0 (Toka)", f"Expected 'trg 0.16.0 (Toka)', got '{r_pkg_ver.stdout.strip()}'"

    r_dir_ver = run_cmd([str(direct_bin_path), "-V"])
    assert r_dir_ver.stdout.strip() == "trg 0.16.0 (Toka)", f"Expected 'trg 0.16.0 (Toka)', got '{r_dir_ver.stdout.strip()}'"
    ...
    assert "trg 0.16.0" in r_re_ctx.stdout
    ...
    try:
        ...
        assert resp1["result"]["serverInfo"]["version"] == "0.16.0"
...
"""
    ...
    with tempfile.TemporaryDirectory(prefix="trg_test_135_") as td:
        ...
        assert len(lines) == 3
        # 1. initialize
        assert lines[0]["result"]["serverInfo"]["version"] == "0.16.0"
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # 1. initialize
        assert resps[0]["result"]["serverInfo"]["version"] == "0.16.0"
    ...
    log("=" * 60)
    log("ALL 174 RIGOROUS QUALIFICATION TESTS PASSED ON PACKAGE ARTIFACT (v0.16.0)!")
    log("=" * 60)
