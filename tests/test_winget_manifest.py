...
class TestWinGetManifestGeneration(unittest.TestCase):
    ...
    def test_winget_manifest_structure_and_values(self):
        r = self.run_generator(version="0.16.1", tag="v0.16.1")
        self.assertEqual(r.returncode, 0, f"Generator failed: {r.stderr}")

        pkg_dir = self.out_dir / "manifests" / "t" / "Tokalang" / "trg" / "0.16.1"
        ...
        self.assertIn("PackageVersion: 0.16.1", ver_text)
        ...
        self.assertIn("trg-v0.16.1-windows-x64\\trg.exe", inst_text)
        self.assertIn("trg-v0.16.1-windows-arm64\\trg.exe", inst_text)
