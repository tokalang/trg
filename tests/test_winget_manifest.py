...
class TestWinGetManifestGeneration(unittest.TestCase):
    ...
    def run_generator(self, version="0.17.0", tag="v0.17.0"):
        cmd = [
            sys.executable,
            str(GENERATOR),
            "--version", version,
    ...
    def test_winget_manifest_structure_and_values(self):
        r = self.run_generator(version="0.17.0", tag="v0.17.0")
        self.assertEqual(r.returncode, 0, f"Generator failed: {r.stderr}")

        pkg_dir = self.out_dir / "manifests" / "t" / "Tokalang" / "trg" / "0.17.0"
        ...
        self.assertIn("PackageVersion: 0.17.0", ver_text)
        ...
        self.assertIn("trg-v0.17.0-windows-x64\\trg.exe", inst_text)
        self.assertIn("trg-v0.17.0-windows-arm64\\trg.exe", inst_text)
